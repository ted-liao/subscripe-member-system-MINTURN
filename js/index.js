...
function updateUserSection() {
    ...
    // --- 1. 處理右上角 User Section (顯示名字與文字版登出) ---
    if (currentUser) {
    ...
    if (currentUser && currentUser.isAdmin) {
        ...
        // 關鍵樣式：margin-left: auto 會把它推到最右邊 (紅色圈圈處)
        ...
        adminBtn.onmouseover = function() {
            this.style.boxShadow = "0 0 20px rgba(189, 0, 255, 0.8)";
            this.style.background = "rgba(189, 0, 255, 0.1)";
        };
        adminBtn.onmouseout = function() {
            this.style.boxShadow = "0 0 10px rgba(189, 0, 255, 0.3)";
